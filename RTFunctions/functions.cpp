...
unsigned int RedTrainer::getCurrentAnimation()
{
	...
	return player->m_pAnimationSlot->m_pBegin[0].field_4;
}
...
unsigned int RedTrainer::getFirstEnemyCurrentAnimation()
{
	...
	return enemy->m_pAnimationSlot->m_pBegin[0].field_4;
}
...
void RedTrainer::setAddWeapons(char weaponNum, int weaponValue)
{
	...
	Core_PlaySound("core_se_sys_decide_s", 0);
	items.m_pBegin[weaponNum]->set(weaponValue);
}
...
void RedTrainer::setAttackType(int attackType) {
	...
	unsigned int playerAttackTypeAddr = 0;
...
void RedTrainer::setMission(short missionId, char missionName[])
{
	...
	if (*(int*)(shared::base + 0x1764670))
	{
		*(int*)(shared::base + 0x1764670) = missionId;
		strcpy((char*)(shared::base + 0x1764674), missionName);
	}
	else
	{
		*(int*)(shared::base + 0x1766004) = missionId;
		strcpy((char*)(shared::base + 0x1766008), missionName);
	}
}
...
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(int*)(shared::base + 0x1764430) = difficultyValue;
}
...
void RedTrainer::setNoDamage(bool& bNoDamage)
{
	...
	if (bNoDamage)
	{
		injector::WriteMemoryRaw(shared::base + 0x81B488, (void*)"\xEB\x05", 2, true);
		injector::WriteMemoryRaw(shared::base + 0x1776218, (void*)"\x00", 1, false);
	}
	else
	{
		injector::WriteMemoryRaw(shared::base + 0x81B488, (void*)"\x74\x05", 2, true);
	}
}
...
void RedTrainer::setNoKilled(bool& bNoKilled) //doesnt work
{
	...
	if (bNoKilled)
	{
		injector::WriteMemoryRaw(shared::base + 0x81B494, (void*)"\xEB\x05", 2, true);
		injector::WriteMemoryRaw(shared::base + 0x177621C, (void*)"\x00\x00\x00\x00", 4, false);
	}
	else
	{
		injector::WriteMemoryRaw(shared::base + 0x81B494, (void*)"\x74\x05", 2, true);
	}
}
...
void RedTrainer::setNoAlert(bool& bNoAlert) //doesnt work
{
	...
	if (bNoAlert)
	{
		injector::MakeNOP(shared::base + 0x82C417, 6, true);
		injector::WriteMemoryRaw(shared::base + 0x1776174, (void*)"\x00\x00\x00\x00", 4, false);
	}
	else
	{
		injector::WriteMemoryRaw(shared::base + 0x82C417, (void*)"\x01\x05\x74\x61\x30\x02", 6, true);
	}
}
...
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(float*)(shared::base + 0x1776204) = timerValue;
}

void RedTrainer::setBattlePoints(int battlePointsValue)
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(int*)(shared::base + 0x1776208) = battlePointsValue;
}

void RedTrainer::setMaxCombo(int maxComboValue)
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(int*)(shared::base + 0x1776210) = maxComboValue;
}

void RedTrainer::setKills(int killsValue)
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(int*)(shared::base + 0x1776214) = killsValue;
}

void RedTrainer::setZandzutsuKills(int zandzutsuKillsValue)
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(int*)(shared::base + 0x177620C) = zandzutsuKillsValue;
}
...
{
	if (((BOOL(__cdecl*)(int))(shared::base + 0x16D50))(raidenFlag))
		((void(__cdecl*)(int))(shared::base + 0x9CC90))(raidenFlag);
	else
		((void(__cdecl*)(int))(shared::base + 0x9CC70))(raidenFlag);
}

void RedTrainer::setRender(unsigned int renderType)
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(int*)(shared::base + 0x17EA09C) = renderType;
}
...
{
	Core_PlaySound("core_se_sys_decide_s", 0);
	float* filter = (float*)(shared::base + 0x1ADD604);
	*(filter + filterOffset) = filterValue;
}
...
void RedTrainer::setSize(float sizeValue, char sizeOffset)
{
	...
	Core_PlaySound("core_se_sys_decide_s", 0);
	*(float*)(player + 0x70 + sizeOffset) = sizeValue;
};
...
void RedTrainer::setMenuType(char menuType)
{
	...
	*(int*)(shared::base + 0x17E9F9C) = menuType;
}
...
void RedTrainer::setAllEnemies(int typeEnemyActive, unsigned int enemyId, unsigned int enemyType, unsigned int enemySetType, unsigned int enemyFlag)
{
	...
	if (typeEnemyActive & 0x1) //delete write functions
	{
		...
		injector::MakeNOP(shared::base + 0x8A1960, 0x11, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1960, (void*)"\xC7\x46\x0C", 3, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1963, (void*)&enemyId, 4, true);

		injector::MakeNOP(shared::base + 0x8A1A4D, 0x11, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1A4D, (void*)"\xC7\x46\x44", 3, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1A50, (void*)&enemyType, 4, true);

		injector::MakeNOP(shared::base + 0x8A1AC5, 0x11, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1AC5, (void*)"\xC7\x46\x50", 3, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1AC8, (void*)&enemyFlag, 4, true);
	}

	if (typeEnemyActive & 0x2) //SetType to nop
	{
		injector::MakeNOP(shared::base + 0x8A1A75, 0x11, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1A75, (void*)"\xC7\x46\x48", 3, true);
		injector::WriteMemoryRaw(shared::base + 0x8A1A78, (void*)&enemySetType, 4, true);
	}
	/*
	...
	{
		injector::WriteMemoryRaw(shared::base + 0x8A1960, (void*)"\x8B\x17\x8D\x4E\x0C\x51\x50\x8B\x82\xE8\x00\x00\x00\x8B\xCF\xFF\xD0", 0x11, true); //reset id
		injector::WriteMemoryRaw(shared::base + 0x8A1A4D, (void*)"\x8B\x17\x8D\x4E\x44\x51\x50\x8B\x82\xD8\x00\x00\x00\x8B\xCF\xFF\xD0", 0x11, true); //reset type
		injector::WriteMemoryRaw(shared::base + 0x8A1A75, (void*)"\x8B\x17\x8D\x4E\x48\x51\x50\x8B\x82\xD8\x00\x00\x00\x8B\xCF\xFF\xD0", 0x11, true); //reset setType
		injector::WriteMemoryRaw(shared::base + 0x8A1AC5, (void*)"\x8B\x17\x8D\x4E\x50\x51\x50\x8B\x82\xE8\x00\x00\x00\x8B\xCF\xFF\xD0", 0x11, true); //reset flag
...
void RedTrainer::enemyNoDamageTo(int damageType) {
	...
	injector::WriteMemoryRaw(shared::base + 0x6C7C80, (BYTE*)&damageType, sizeof(damageType), true);
}
...
void RedTrainer::setBackground(char bgType, bool enabled)
{
	...
	if (bgType == 0 && enabled)
	{
		...
		injector::WriteMemoryRaw(0x17EA138 + shared::base, (BYTE*)&resultVar, sizeof(resultVar), true);
...
void RedTrainer::setDynamicWeapon(bool isEnabled, unsigned int weaponId)
{
	...
	{
		///Changed weapon type
		injector::WriteMemoryRaw(shared::base + 0x7885C0, (BYTE*)"\xBF", 1, true);
		injector::WriteMemoryRaw(shared::base + 0x7885C1, (BYTE*)&weaponId, sizeof(weaponId), true);

		///Disabling verification func 
		injector::MakeNOP(shared::base + 0x592DE9, 2, true);

		///WeaponMenuType 2 -> 4
		//mem::in::read((BYTE*)(shared::base + 0x8294B5, (BYTE*)&buffer, sizeof(buffer));
		injector::WriteMemoryRaw(shared::base + 0x8294AD, (BYTE*)"\x83\x05", 2, true);
		injector::WriteMemoryRaw(shared::base + 0x8294B3, (BYTE*)"\x02", 1, true);
		injector::MakeNOP(shared::base + 0x8294B4, 5, true);

		///Change weaponMenuType shared::base + 17EA124 to 2 (write pointer to object in global var)
		injector::WriteMemoryRaw(shared::base + 0x17EA124, (BYTE*)"\x02", 1, true);
	...
	if (!isEnabled)
	{
		...
		//weapon menu type
		injector::WriteMemoryRaw(shared::base + 0x8294AD, (BYTE*)"\x01\x1D", 2, true);
		injector::WriteMemoryRaw(shared::base + 0x8294B5, (BYTE*)&funcAddr, sizeof(funcAddr), true);
		injector::WriteMemoryRaw(shared::base + 0x8294B3, (BYTE*)"\x89\x1D", 2, true);

		//jne
		injector::WriteMemoryRaw(shared::base + 0x592DE9, (BYTE*)"\x75\x07", 2, true);

		//weapon id
		injector::WriteMemoryRaw(shared::base + 0x7885C0, (BYTE*)"\x75\x05\x83\xCF\xFF", 5, true);
	}
